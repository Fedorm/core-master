...
using JsException = BitMobile.Application.Exceptions.JsException;
...
namespace Jint
{
    ...
    public class JintEngine
    {
        ...
        public String CurrentLine
        {
            get
            {
                if (this.visitor.CurrentStatement != null)
                    if (this.visitor.CurrentStatement.Source != null)
                        return this.visitor.CurrentStatement.Source.ToString();
                return "";
            }
        }
        ...
        public String CurrentString
        {
            get
            {
                if (this.visitor.CurrentStatement != null)
                    if (this.visitor.CurrentStatement.Source != null)
                        return this.visitor.CurrentStatement.Source.Code;
                return "";
            }
        }

        [System.Diagnostics.DebuggerStepThrough]
        protected JintEngine()
            : this(Options.Ecmascript5 | Options.Strict)
        {
        }

        [System.Diagnostics.DebuggerStepThrough]
        protected JintEngine(Options options)
        {
            visitor = new ExecutionVisitor(options, this as IScriptEngineContext, this);
            AllowClr = true;
        ...
        public virtual void WriteToDebugConsole(string name, params object[] args)
        {
        }
        ...
        {
            if (DebugMode)
                WriteToDebugConsole(name, args);

            object func = visitor.CurrentScope[name];
            if (func.GetType() == typeof(JsUndefined))
                throw new Exception(String.Format("Unknown function '{0}'", name));
            return CallFunction((JsFunction)func, args);
        }
        ...
        public object CallFunction(JsFunction function, params object[] args)
        {
            ...
            catch (TargetInvocationException e)
            {
                ...
                throw new JsException(message, e);
            }
            catch (JsGlobalException e)
            {
        ...
        {
            object var = visitor.CurrentScope[varName];
            if (var.GetType() == typeof(Jint.Native.JsUndefined))
                return null;
            else
                return ((JsObject)var).Value;
        }
