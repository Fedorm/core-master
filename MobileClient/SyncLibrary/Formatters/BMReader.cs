...
namespace Microsoft.Synchronization.Services.Formatters
{
    public class BMReader : SyncReader
    {
        ...
        public BMReader(Stream stream, EntityType[] knownTypes)
        ...
        public override IsolatedStorageOfflineEntity GetItem()
        {
            CheckItemType(ReaderItemType.Entry);

            // Get the type name and the list of properties.
            _currentEntryWrapper = new BMEntryInfoWrapper((XElement)XNode.ReadFrom(_reader));

            _liveEntity = CreateEntity(_currentEntryWrapper, _knownTypesDict);
        ...
        public override bool Next()
        {
            ...
            do
            {
                ...
                if (_reader.NodeType == XmlNodeType.Element &&
                    (_reader.LocalName == BMFormatterConstants.Entry
                    || _reader.LocalName == BMFormatterConstants.Tombstone))
                {
                    _currentType = ReaderItemType.Entry;
                    _currentNodeRead = false;
                    return true;
                }
                if (_reader.NodeType == XmlNodeType.Element && _reader.LocalName == FormatterConstants.ServerBlobText)
                {
                    _currentType = ReaderItemType.SyncBlob;
                    _currentNodeRead = false;
                    return true;
                }
                if (_reader.NodeType == XmlNodeType.Element && _reader.LocalName == FormatterConstants.MoreChangesAvailableText)
                {
