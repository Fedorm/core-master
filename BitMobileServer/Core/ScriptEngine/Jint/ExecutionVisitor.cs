    ...
    public class ExecutionVisitor : IStatementVisitor, IJintVisitor, IDeserializationCallback
    {
            ...
            if (temp.IsClr) // && ((JsValue)Result).Type == JsValueType.CLRObject)
            {
                ...
                try
                {
                    ...
                    else
                    {
                        ...
                        {
                            if (temp.Value is BitMobile.ValueStack.IIndexedProperty)
                            {
                                BitMobile.ValueStack.IIndexedProperty iProp = (BitMobile.ValueStack.IIndexedProperty)temp.Value;
                                if (iProp.HasProperty(Result.ToString()))
                                {
                                    Result = Global.WrapClr(iProp.GetValue(Result.ToString()));
                                    return;
                                }
                            }
                        }
        ...
        public void Visit(PropertyExpression expression)
        {
            ...
            if (CurrentScope.IsClr && CurrentScope.Value != null)
            {
                ...
                if (CurrentScope.Value is BitMobile.ValueStack.IIndexedProperty)
                {
                    BitMobile.ValueStack.IIndexedProperty iProp = (BitMobile.ValueStack.IIndexedProperty)CurrentScope.Value;
                    if (iProp.HasProperty(propertyName))
                    {
                        Result = Global.WrapClr(iProp.GetValue(propertyName));
                        return;
                    }
                }
