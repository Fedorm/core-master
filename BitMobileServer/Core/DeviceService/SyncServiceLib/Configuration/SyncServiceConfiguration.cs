...
namespace Microsoft.Synchronization.Services
{
    ...
    internal sealed class SyncServiceConfiguration : ISyncServiceConfiguration
    {
        ...
        //internal SyncSerializationFormat SerializationFormat = SyncSerializationFormat.ODataJson;
        ...
        internal void Initialize(Type syncServiceType, string dataBaseName)
        {
            if (!IsInitialized)
            {
                lock (_lockObject)
                {
                    if (!IsInitialized)
                    {
                        ...
                        InvokeStaticInitialization(syncServiceType, dataBaseName);
        ...
        private void InvokeStaticInitialization(Type type, string dataBaseName)
        {
            ...
            while (type != null)
            {
                MethodInfo info = type.GetMethod("InitializeService", bindingAttr, null, new[] { typeof(ISyncServiceConfiguration),typeof(string) }, null);

                if ((info != null) && (info.ReturnType == typeof(void)))
                {
                    ParameterInfo[] parameters = info.GetParameters();

                    if ((parameters.Length == 2) && !parameters[0].IsOut)
                    {
                        var objArray = new object[] { this,dataBaseName };
