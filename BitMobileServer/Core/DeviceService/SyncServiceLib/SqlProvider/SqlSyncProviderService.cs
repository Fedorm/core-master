...
namespace Microsoft.Synchronization.Services.SqlProvider
{
    ...
    internal sealed class SqlSyncProviderService : IAsymmetricProviderService
    {
        ...
        private GetChangesResponse GetChanges(byte[] serverBlob, Guid batchCode, Guid nextBatchSequenceNumber, Dictionary<string,object> providerParams)
        {
            ...
            if (null == batch)
            {
                ...
                return GetChanges(serverBlob, providerParams);
            }
        ...
        private void BeforeSyncSession(byte[] serverBlob, string scopeName, string connectionString, Dictionary<string,object> providerParams)
        {
            using (SqlConnection conn = new SqlConnection(connectionString))
            {
                conn.Open();
                using (SqlCommand cmd = new SqlCommand("[admin].BeforeSyncSession", conn))
                {
                    try
                    {
                    ...
                    String anchor = BitConverter.ToString(serverBlob);
                    cmd.Parameters["@anchor"].Value = anchor.Replace("-", "");
                    cmd.Parameters["@scope"].Value = scopeName;

                    foreach (KeyValuePair<String, object> kvp in providerParams)
                    {
        ...
        /// <param name="serverBlob">Client knowledge as byte[]</param>
        /// <returns>Response containing the new knowledge and the list of changes.</returns>
        public GetChangesResponse GetChanges(byte[] serverBlob, Dictionary<string,object> providerParams)
        {
            ...
            else
            {
                ...
                if (null != incomingBlob.BatchCode && null != incomingBlob.NextBatch)
                {
                    ...
                    return GetChanges(incomingBlob.ClientKnowledge, incomingBlob.BatchCode.Value, incomingBlob.NextBatch.Value, providerParams);
                }
            }
            ...
            try
            {
                ...
                BeforeSyncSession(serverBlob, _clientScopeName, _serverConnectionString, providerParams);
        ...
        private static SqlSyncProvider CreateSqlSyncProviderInstance(string clientScopeName, string serverConnectionString, string syncObjectSchema)
        {
            ...
            sqlSyncProvider.CommandTimeout = 300;
