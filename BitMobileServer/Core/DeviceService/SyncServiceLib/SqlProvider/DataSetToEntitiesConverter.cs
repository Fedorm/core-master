...
namespace Microsoft.Synchronization.Services.SqlProvider
{
    ...
    internal sealed class DataSetToEntitiesConverter
    {
        ...
        internal string GetSelectScriptForType(Type t)
        {
            ...
            {
                colsList.Append(delimiter).Append(String.Format("[{0}]",(mappingInfo.ContainsKey(pinfo.Name)) ? mappingInfo[pinfo.Name] : pinfo.Name));
            ...
            {
                pkList.Append(delimiter).Append(String.Format("[{0}]",(mappingInfo.ContainsKey(pinfo.Name)) ? mappingInfo[pinfo.Name] : pinfo.Name));
            ...
            if (arr.Length == 2)
            {
                tableName = String.Format("[{0}].[{1}]", arr[0], arr[1]);
            }
