...
namespace RemObjects.InternetPack
{
    ...
    public class Connection : Stream, IDisposable, IConnectionTimeouts
    {
        ...
        protected virtual void DataSocketConnect(EndPoint endPoint)
        {
            ...
            IAsyncResult ar = DataSocket.BeginConnect(endPoint, ConnectCompleted, evt);
            try
            {
                if (!evt.WaitOne(5000)) //5 sec
                    throw new System.Net.WebException("Connection timeout");
            }
            catch
            {
                throw new System.Net.WebException("Connection timeout");
        ...
        private void ConnectCompleted(IAsyncResult ar)
        {
            try
            {
                System.Threading.AutoResetEvent evt = (System.Threading.AutoResetEvent)ar.AsyncState;
            
                if (ar.IsCompleted)
                    evt.Set();
                evt.Close();
                DataSocket.EndConnect(ar);
            }
            catch
            {
            }
