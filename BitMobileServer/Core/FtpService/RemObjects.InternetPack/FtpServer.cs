...
namespace RemObjects.InternetPack.Ftp
{
    ...
    public class FtpServer : CommandBasedServer
    {
        ...
        {
            this.Port = 21;
            this.PassivePortFrom = 50000;
            this.PassivePortTo = 50019;
        ...
        */
        ...
        {
            FtpSession lSession = (FtpSession)e.Session;
            FtpServer lServer = (FtpServer)sender;
            ...
            if (lSession.State == FtpState.LoggedIn)
            {
                ...
                if (lSession.PassiveServer == null)
                {
                    //IPAddress address = String.IsNullOrEmpty(lServer.PassiveAddress) ? ((IPEndPoint)e.Connection.LocalEndPoint).Address : IPAddress.Parse(lServer.PassiveAddress);
                    lSession.PassiveServer = new PassiveServer(((IPEndPoint)e.Connection.LocalEndPoint).Address, lServer.PassivePortFrom, lServer.PassivePortTo);
...
#endif
                if (!String.IsNullOrEmpty(lServer.PassiveAddress))
                    lAddress = IPAddress.Parse(lServer.PassiveAddress).GetAddressBytes();
