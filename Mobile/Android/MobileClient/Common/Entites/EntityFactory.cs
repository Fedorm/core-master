...
namespace BitMobile.Common.Entites
{
    public static class EntityFactory
    {
        ...
        public static Func<EntityType, IEntity> CreateInstance { get; set; }
        ...
        private static EntityType CreateEntityType(XmlNode node)
        {
            ...
            var fields = new List<EntityField>(fieldNodes.Count);
            for (int i = 0; i < fieldNodes.Count; i++)
                fields.Add(CreateEntityField(fieldNodes[i], i, ToTableName(schema, name)));

            return new EntityType(schema, name, fields);
        }
