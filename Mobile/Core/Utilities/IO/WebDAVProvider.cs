...
{
    public class WebDAVProvider : Provider, IRemoteProvider
    {
        ...
        public override void SaveFile(string relativePath, Stream source)
        {
            ...
            var item = new Item { RelativePath = relativePath, Time = DateTime.UtcNow };
            Items.Add(item);
        }
        ...
        {
            if (!Items.Exists(val => val.RelativePath.StartsWith(path)))
                try
                {
