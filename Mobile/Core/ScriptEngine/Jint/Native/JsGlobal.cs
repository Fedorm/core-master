...
namespace Jint.Native
{
    ...
    public class JsGlobal : JsObject, IGlobal
    {
        ...
        public JsInstance GetType(JsInstance[] arguments)
        {
            ...
            string type = arguments[0].Value.GetType().ToString();

            return new JsString(type);
        }
        ...
        public JsInstance Format(JsInstance[] arg)
        {
            throw new JsGlobalException(
                "Function Format not implemented. You should use String.Format(<input string>, <parameters>).");
        }
        #endregion
        ...
        static void CheckArg(JsInstance[] arg, int expectedCount, bool throwIfNull = true)
        {
            ...
            for (int i = 0; i < arg.Length; i++)
            {
                if (arg[i] == JsUndefined.Instance)
                    throw new JsGlobalException("Argument at index {0} has undefined type".Format(i));

                if (throwIfNull)
                    if (arg[i].Value == null)
                        throw new JsGlobalException("Argument at index {0} is null".Format(i));
            }
        }
        ...
        static string ParseStr(JsInstance obj)
        {
            ...
            if (!TryParseString(obj, out result))
                throw new JsGlobalException("Invalid cast '{0}' to 'String'".Format(obj));
        ...
        static int ParseInt(JsInstance obj)
        {
            ...
            if (!TryParseNum(obj, out result))
                throw new JsGlobalException(
                    "Invalid cast '{0}' to 'Int'. Js object: {1}".Format(obj.ToObject(), obj));
            return (int)result;
        }
        ...
        static DateTime ParseDate(JsInstance obj)
        {
            ...
            if (!TryParseDate(obj, out result))
                throw new JsGlobalException(
                    "Invalid cast '{0}' to 'DateTime'. Js object: {1}".Format(obj.ToObject(), obj));
            return result;
        }
