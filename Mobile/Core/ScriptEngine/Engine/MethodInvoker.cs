...
{
    class MethodInvoker : IMethodInvoker
    {
        ...
        /*
        private MethodInfo GetAsObject(object subject, string method, ref object[] parameters)
        {
            MethodInfo mi = subject.GetType().GetMethod(method);
            if (mi != null)
                parameters = new object[1] { _engine.Stack.Values["dao"] };
            return mi;
        }
        */

        public MethodInfo Invoke(object subject, string method, ref object[] parameters, Type[] generics)
        {
            MethodInfo methodInfo = null;

            if (subject is BitMobile.ValueStack.IGlobalModule)
                return (subject as BitMobile.ValueStack.IGlobalModule).GetProxyMethod(method, parameters);

//            if (method.EndsWith("AsObject") && parameters.Length == 0)
//                methodInfo = GetAsObject(subject, method, ref parameters);
            if (methodInfo != null)
                return methodInfo;
            ...
            for (var i = 0; i < pis.Length; i++)
            {
                ...
                try
                {
                    ...
                    */
                    ...
                    {
                        Type t = Nullable.GetUnderlyingType(pis[i]) ?? pis[i];
                        parameters[i] = Convert.ChangeType(parameters[i], t);
                    }
                }
                catch
                {
